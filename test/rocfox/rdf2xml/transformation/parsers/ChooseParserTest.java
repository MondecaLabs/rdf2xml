...
public class ChooseParserTest
{
	...
	public void isElement()
	{
		...
		ChooseParser parser = new ChooseParser(new Transform());
	...
	public void parse()
	{
		Document document = DocumentReader.readFromJar(getClass().getCanonicalName().replace('.', '/') + ".xml");
		ChooseParser parser = new ChooseParser(new Transform());
		Choose choose = parser.parse(document.getDocumentElement());
		Assert.assertEquals(2, choose.getWhen().size());
		for(When when : choose.getWhen())
		{
			Assert.assertTrue("Test1".equals(when.getTest().getName()) || "Test2".equals(when.getTest().getName())) ;
		}
		Assert.assertNotNull(choose.getOtherwise());
	}
}
