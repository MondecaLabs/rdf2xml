...
public class ParamParserTest
{
	...
	public void isElement()
	{
		...
		ParamParser parser = new ParamParser(new Transform());
	...
	public void parse()
	{
		Document document = DocumentReader.readFromJar(getClass().getCanonicalName().replace('.', '/') + ".xml");
		ParamParser parser = new ParamParser(new Transform());
		Param result = parser.parse(document.getDocumentElement());
		Assert.assertEquals("Param1", result.getName());
		Assert.assertTrue(result instanceof ParamLiteral);
	}
}
