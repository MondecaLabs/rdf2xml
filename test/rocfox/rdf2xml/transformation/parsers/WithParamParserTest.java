...
public class WithParamParserTest
{
	...
	public void parse()
	{
		Document document = DocumentReader.readFromJar(getClass().getCanonicalName().replace('.', '/') + ".xml");
		WithParamParser parser = new WithParamParser(new Transform());
		WithParam result = parser.parse(document.getDocumentElement());
		Assert.assertEquals("MyParam", result.getName());
	}
}
