 ...
 * @author Nicolas
 */
public class TemplateOutputParserTest
{
	@Test
	public void isElement()
	{
		Document document = DocumentReader.readFromJar(getClass().getCanonicalName().replace('.', '/') + ".xml");
		TemplateOutputParser parser = new TemplateOutputParser(new Transform());
	...
	public void parse()
	{
		Document document = DocumentReader.readFromJar(getClass().getCanonicalName().replace('.', '/') + ".xml");
		TemplateOutputParser parser = new TemplateOutputParser(new Transform());
		TemplateOutput result = parser.parse(document.getDocumentElement());
		//more test here to be writen
	}
}
