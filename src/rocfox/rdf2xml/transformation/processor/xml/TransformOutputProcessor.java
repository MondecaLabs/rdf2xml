 ...
 * @author Nicolas
 */
public class TransformOutputProcessor
{
	private TransformOutput input;
	
	private Model model;
	
	public TransformOutputProcessor(Model model, TransformOutput input)
	{
		if(input == null)
		{
	...
	public void transform(XmlWriter writer, TransformContext context)
	{
		...
		writer.writeStartDocument(input.getEncoding(), input.getVersion());
		rocfox.rdf2xml.transformation.processor.xml.NodesProcessor processor = new rocfox.rdf2xml.transformation.processor.xml.NodesProcessor(model, input.getChildren());
